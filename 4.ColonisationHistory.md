...
```
## 4. Edit and launch script to run several itirationso of FSC
```
Edit [FSC.FULLModel.ARRAY.sh](https://github.com/alexjvr1/AriciaAgestis_PopGenMS/blob/master/FSC.FULLModel.ARRAY.sh). 

Specify Model names and input file (tpl and est) names. 

To create an Array job we need a list of job names. e.g for 100 names: 
```
#seq [increment] [range]

seq 1 100 >> numRunSeq

#add "Run" to the start of each line
sed -i 's/^/Run/g' numRunSeq
...
```
qsub FSC.FULLModel.ARRAY.sh -F "BA Model1 Model1_ DSFS.obs 100"
```
