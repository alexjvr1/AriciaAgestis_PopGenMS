...
```
We need several 1000 loci for fastSimCoal to work effectively. See Laurent Excoffier's comment [here](https://groups.google.com/forum/#!topic/fastsimcoal/E_GdjQCGPtU). The current dataset doesn't lend itself to this analysis. One option is to downsample the number of individuals in the dataset. A second option is to partition the dataset into the different groups and then impute the missing data within each dataset. 
