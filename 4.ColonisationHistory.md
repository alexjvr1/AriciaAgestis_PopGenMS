...
2. To exclude linked loci we first calculate mean linkage (r^2) between all tags, then thinned the data accordingly. 

3. Populations are downsampled to exclude all missing data using a python script available from the paper. 

This was done on the server
```
/newhome/aj18951/1a_Aricia_agestis_PopGenomics/FastSimCoal

vcftools --vcf AA251.FINAL.MAF0.01.missing0.5perpop.vcf
...
First remove all loci under selection. For this I need to identify a list of outliers for 1) host plant preference, and 2) colonisation history. Three of these overlaps. I'll remove the entire ddRAD tag contains outlier loci. These vcf files have already been generated for fineRADstructure: 
```
pwd
/newhome/aj18951/1a_Aricia_agestis_PopGenomics/FastSimCoal
...
vcftools --vcf AA251.FINAL.MAF0.01.missing0.5perpop.vcf --exclude-bed outliers_toremove.bed --recode --recode-INFO-all --out AA251.FINAL.MAF0.01.missing0.5perpop.Neutral

	Read 26 BED file entries.
After filtering, kept 60994 out of a possible 61210 Sites
```

Downsample to one SNP per locus to minimise linkage disequilibrium in the dataset. RAD tags are max 600bp, but linkage blocks could be larger. I'm using --thin to downsample to one SNP per 10kb based on a plot of r2 on Chr 9 (see below).  
```
vcftools --vcf AA251.FINAL.missing0.5perpop.Neutral.recode.vcf --thin 10000 --recode --recode-INFO-all --out AA251.Neutral.thin

After filtering, kept 251 out of 251 Individuals
Outputting VCF file...
After filtering, kept 8651 out of a possible 136291 Sites
Run Time = 4.00 seconds


##If thinning for 600bp (i.e. size of RAD tags) we get 10864 loci. 

##And remove Z-chromosome

vcftools --vcf AA251.Neutral.thin.recode.vcf --not-chr SUPER_Z --recode --recode-INFO-all --out AA251.Neutral.noZ.thin

VCFtools - 0.1.17
(C) Adam Auton and Anthony Marcketta 2009

Parameters as interpreted:
	--vcf AA251.Neutral.thin.recode.vcf
	--not-chr SUPER_Z
	--recode-INFO-all
	--out AA251.Neutral.noZ.thin
	--recode

After filtering, kept 251 out of 251 Individuals
Outputting VCF file...
After filtering, kept 7961 out of a possible 8651 Sites
Run Time = 5.00 seconds


##Vitor suggests we need at least 8000 loci. We'll try with 7961. 
```


### LD

Calculate the mean linkage (r^2) between loci using vcftools
