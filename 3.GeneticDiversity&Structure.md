...
```
/newhome/aj18951/1a_Aricia_agestis_PopGenomics/DiversityStats

vcftools --vcf ../FINAL_VCF/AA251.FINAL.MAF0.01.missing0.5perpop.vcf --het

vcftools --vcf ../FINAL_VCF/AA251.FINAL.MAF0.01.missing0.5perpop.vcf --SNPdensity 1000
...
AA251.pop <- read.table("AA.251.names", header=F) ## add population information
AA251.pop$pop <- gsub("_.*_.*", "", AA251.pop$V1)
...
ggplot(pop.AA251, aes(x=PC1, y=PC2, colour=Colour, shape=Shape, size=1)) + geom_point() + scale_shape_manual(values = c(15,17)) + scale_color_manual(values=c("gold1", "goldenrod1", "darkorchid3", "darkorchid4"))
dev.off()
